...
static int migration_cpu_stop(void *data)
{
	...
	if (task_rq(p) == rq) {
		...
		if (dest_cpu < 0) {
			 ...
			 */
			if (!pending) {
				WARN_ON_ONCE(!cpumask_test_cpu(task_cpu(p), &p->cpus_allowed));
				goto out;
			}

			dest_cpu = cpumask_any_distribute(&p->cpus_allowed);
		}
		 ...
		 */
		if (pending && cpumask_test_cpu(task_cpu(p), &p->cpus_allowed)) {
			p->migration_pending = NULL;
			complete = true;
			goto out;
		}
	...
	__releases(p->pi_lock)
{
	...
	if (!(ctx->flags & SCA_MIGRATE_ENABLE)) {
		if (cpumask_equal(&p->cpus_allowed, ctx->new_mask)) {
			if (ctx->flags & SCA_USER)
				swap(p->user_cpus_ptr, ctx->user_mask);
			goto out;
		}
